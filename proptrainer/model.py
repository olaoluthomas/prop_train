...
from vertex_proptrainer import features
...
def get_credentials():
    ...
    from vertex_proptrainer import key
...
def get_class_weights(y_train):
    """
    Function to get the weights of each class in target based on
    class frequencies.
    ...
    -------
    dict
        dictionary with classes as keys and class weights as values.
    """
...
def get_estimator(y_train, model_type, n_rounds, random_state=777, **kwargs):
    ...
    Options: XGBoostClassifier, SGDClassifier.
    ...
    model_type: str
        ...
        be a Stochastic Gradient Descent Classifier using log loss.
    n_rounds: int
        # of training rounds without evaluation improvement
    ...
    -------
    estimator
        ML estimator with hyperparameters initialized
...
def calibrate(estimator, X_test, y_test):
    """
    Calibrate the estimator to ensure probabilities are indicative of
    ...
    calibrated_estimator
        Estimator that supports calibtrated probability estimates.
    """
...
def train_and_evaluate(segment,
    ...
    Returns
    -------
    calibrated_estimator: Calibrated ML estimator (sklearn OR xgboost).
